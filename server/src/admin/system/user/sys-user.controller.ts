import { BadRequestException, Body, Controller, Delete, Get, Param, ParseArrayPipe, ParseIntPipe, Post, Put, Query, Req, Res, UploadedFile, UseInterceptors } from '@nestjs/common';
import { ParseIntArrayPipe } from '@/common/pipe/parse-int-array.pipe';
import Result from '@/common/result/Result';
import { ApiBearerAuth, ApiBody, ApiOperation, ApiQuery, ApiResponse, ApiTags } from '@nestjs/swagger';
import { RequirePermission } from '@/common/decorator/require-premission.decorator';
import { nowDate, nowDateTime, tree } from '@/common/utils';
...
export class SysUserController {
    ...
    async getRolePost() {
        ...
        let posts = await this.prisma.sysPost.findMany()
        return {
            ...Result.ok(),
            roles, posts
        }
    ...
    async listDept(@Query() q: QuerySysDeptDto): Promise<Result<SysDept[]>> {
        let depts = (await this.deptService.selectDeptList(q)).map(v => ({ ...v, id: v.deptId, label: v.deptName }))
    ...
    async getAuthRole(@Param("userId", ParseIntPipe) userId: number): Promise<any> {
        let userInfo = await this.userService.getAuthRole(userId)
        let roles = (await this.prisma.sysRole.findMany({
            where: {
                roleId: {
                    not: 1
                }
    ...
    async getUserProfile(@Req() req): Promise<any> {
        let userInfo = await this.userService.getUserProfile(req.userId)
        return {
            ...Result.ok(userInfo),
    ...
    async getUser(@Param("userId", ParseIntPipe) userId: number): Promise<any> {
        let user = await this.userService.selectUserByUserId(userId)
        let roles = await this.prisma.sysRole.findMany({ where: { roleId: { not: 1 } } })
        let posts = await this.prisma.sysPost.findMany()
        return {
            ...Result.ok(user),
    ...
    async delUser(@Req() req, @Param("ids", ParseIntArrayPipe) userIds: number[]): Promise<Result<any>> {
        ...
        let { count } = await this.userService.deleteUserByUserIds(userIds)
    ...
    @RequireRole("admin")
    @Put("/resetPwd")
    async resetPassword(@Body() sysUser: resetPasswordDto, @Req() req): Promise<Result<any>> {
    ...
    @RequireRole("admin")
    @Put("/changeStatus")
    async updateUserStatus(@Body() sysUser: UpdateSysUserStatusDto, @Req() req): Promise<Result<any>> {
