...
export class SysMenuController {
    ...
    @RequirePermission("system:menu:query")
    @Get("/:menuId")
    async getMenu(@Param("menuId", ParseIntPipe) menuId: number): Promise<Result<SysMenu>> {
    ...
    @Post("/")
    async addMenu(@Body() sysMenu: CreateSysMenuDto, @Req() req): Promise<Result<SysMenu>> {
        sysMenu = {
            ...sysMenu,
    ...
    @RequirePermission("system:menu:edit")
    @Put("/")
    async updateMenu(@Body() sysMenu: UpdateSysMenuDto, @Req() req): Promise<Result<any>> {
        sysMenu = {
            ...sysMenu,
            updateTime: nowDateTime(),
            updateBy: req.user?.userName
    ...
    @Delete("/:ids")
    async delMenu(@Param("ids", ParseIntArrayPipe) menuIds: number[]): Promise<Result<any>> {
        let { count } = await this.menuService.deleteMenuByMenuIds(menuIds)
