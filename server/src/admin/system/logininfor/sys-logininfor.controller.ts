...
export class SysLogininforController {
    ...
    @Get("/list")
    async listLogininfor(@Query() q: QuerySysLogininforDto): Promise<TableDataInfo<SysLogininfor>> {
    ...
    @RequirePermission("monitor:logininfor:query")
    @Get("/:infoId")
    async getLogininfor(@Param("infoId", ParseIntPipe) infoId: number): Promise<Result<SysLogininfor>> {
    ...
    @Post("/")
    async addLogininfor(@Body() sysLogininfor: CreateSysLogininforDto, @Req() req): Promise<Result<SysLogininfor>> {
    ...
    @Put("/")
    async updateLogininfor(@Body() sysLogininfor: UpdateSysLogininforDto, @Req() req): Promise<Result<any>> {
    ...
    @Delete("/:ids")
    async delLogininfor(@Param("ids", ParseIntArrayPipe) infoIds: number[]): Promise<Result<any>> {
        let { count } = await this.logininforService.deleteLogininforByInfoIds(infoIds)
