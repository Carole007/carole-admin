...
export class AuthService {
  ...
  async login(loginBody: LoginBody, req: Request) {
    let { username, password, code, uuid } = loginBody
    ...
    //用户登录日志
    {
      ...
      try {
        if (req.ip?.includes("127.0.0.1") || req.ip == "::1") {
          ...
          let d = qqwry.searchIP(req.ip)
          loginInfo.loginLocation = d.Country
        }
      } catch (e) { }
      let agent = req.headers["user-agent"]
      try {
        let d = bowser.parse(agent)
    ...
    //登录验证码是否开启
    {
      ...
      if (captchaEnabled) {
        ...
        let isPass = await this.validaCaptcha(uuid, code)
        if (!isPass) {
          loginInfo.msg = "验证码错误！"
    ...
    //ip是否被封禁
    {
      let ips = (await redisUtils.get(Constants.SYS_CONFIG_KEY + "sys.login.blackIPList")).split(",")
      if (ips.length) {
        let isBlack = ips.some(v => v === req.ip)
        if (isBlack) {
    ...
    //验证用户密码是否正确
    {
      ...
      let token = this.createToken(cacheInfo)
      //存储token
  ...
  async getUserInfo(userId: number) {
    //如果redis有用户信息直接返回
    let userinfo: SysUser & { roles: string[], permissions: string[] } = JSON.parse(await redisUtils.get(Constants.LOGIN_CACHE_TOKEN_KEY + userId) || null)
    ...
    if (!user) return null
    let ro = user.roles.map(v => v.role).filter(v => v.status === "1")
    let permissions = await this.getRolePermission(ro);
    let result = {
      ...user,
  ...
  async getRouters(userId: number) {
    ...
    if (await this.isAdmin(userId)) {
      let r = await this.prisma.sysMenu.findMany({
        where: {
          menuType: {
            in: ["M", "C"]
          },
      ...
      let r: any = await this.prisma.$queryRaw`select distinct m.menu_id as menuId, m.parent_id as parentId, m.menu_name as menuName, m.path, m.component, m.query, m.visible, m.status, ifnull(m.perms,'') as perms, m.is_frame as isFrame, m.is_cache as isCache, m.menu_type as menuType, m.icon, m.order_num as orderNum, m.create_time as createTime
      from sys_menu m
  ...
  async getRolePermission(roles: SysRole[]) {
    ...
    for (let i = 0; i < roles.length; i++) {
      let r: { perms: string }[] = await this.prisma.$queryRaw`select distinct a.perms from sys_menu a 
  ...
  async validaCaptcha(uuid: string, code: string) {
    let r = await redisUtils.get(Constants.CAPTCHA_CODE_KEY + uuid)
    if (r != code) {
